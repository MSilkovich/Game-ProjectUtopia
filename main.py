...
def set_text(text, x, y):
    ...
    string_rendered = font.render(text, 1, (255, 255, 255))
    intro_rect = string_rendered.get_rect()
    intro_rect.top = y
    intro_rect.x = x
    screen.blit(string_rendered, intro_rect)
...
def Revolt(chanse):
    chanse = chanse
    revolt = random.randint(0, 100)
    if revolt < chanse:
        global structures, foodplus, woodplus, stoneplus, ironplus, grids, goldplus
        structures[-1][0].kill()
        if structures[-1][-1] == 'farm':
            foodplus -= 10
        elif structures[-1][-1] == 'mill':
            woodplus -= 10
        elif structures[-1][-1] == 'iron_mine':
            ironplus -= 10
        elif structures[-1][-1] == 'quarry':
            stoneplus -= 10
        elif structures[-1][-1] == 'goldmine':
            goldplus -= 10
        grids[structures[-1][3]] = True
        del structures[-1]
    ...
    def render(self, screen):
        ...
        if self.mouse_in(mousePos):
            ...
            if pygame.mouse.get_pressed()[0] and self.mouse_in(mousePos):
                global farm, food, wood, type_farm, can_build, grids, board, gold, structures_save
                if food >= 50 and wood >= 100 and gold >= 25:
                    ...
                    structures.append((farm, self.pos[0], self.pos[1], self.cell, 'farm'))
...
class BuildMill(BUildFarm):
    ...
    def render(self, screen):
        ...
        if self.mouse_in(mousePos):
            pygame.draw.rect(screen, (103, 0, 0), (self.position[0], self.position[1], self.width, self.heigth))
            if pygame.mouse.get_pressed()[0] and self.mouse_in(mousePos):
                global mill, food, wood, type_mill, can_build, grids, board, gold
                if food >= 200 and wood >= 25 and gold >= 25:
...
class BuildIronMine(BUildFarm):
    ...
    def render(self, screen):
        ...
        if self.mouse_in(mousePos):
            pygame.draw.rect(screen, (103, 0, 0), (self.position[0], self.position[1], self.width, self.heigth))
            if pygame.mouse.get_pressed()[0] and self.mouse_in(mousePos):
                global ironmine, food, wood, type_ironmine, can_build, grids, board, stone, gold
                if food >= 175 and wood >= 125 and stone >= 100 and gold >= 75:
...
class BuildQuarry(BUildFarm):
    ...
    def render(self, screen):
        ...
        if self.mouse_in(mousePos):
            pygame.draw.rect(screen, (103, 0, 0), (self.position[0], self.position[1], self.width, self.heigth))
            if pygame.mouse.get_pressed()[0] and self.mouse_in(mousePos):
                global quarry, food, wood, type_quarry, can_build, grids, board, gold
                if food >= 175 and wood >= 125 and gold >= 50:
            ...
            if board.grid_pos not in mountains:
                ...
                if self.side == 1:
                    ...
                    pygame.draw.polygon(self.screen, (255, 0, 0), [[self.lx, self.y],
                                                                   ...
                                                                   [self.x, self.y]], 1)
                pygame.draw.rect(self.screen, (0, 0, 0), (self.x, self.y, self.width, self.heigth))
                butmine = BuildIronMine(position=(self.x + 10, self.y + 5), butHeigth=120, butWidth=175, text='')
                ...
                butqu = BuildQuarry(position=(self.x + 10, self.y + 125), butHeigth=90, butWidth=175, text='')
                butqu.render(self.screen)
                self.screen.blit(self.fdres, (self.x + 120, self.y + 130))
                self.screen.blit(self.wdres, (self.x + 120, self.y + 160))
                self.screen.blit(self.gldres, (self.x + 120, self.y + 190))
...
pygame.time.set_timer(type_revolt, 20000)
...
structures.append((castle, 960, 650, castle_sprites))
...
for i in structures_save:
    if structures_save[i] != 0:
        a = structures_save[i]
        if a[0] == 'farm':
            all_sprites.add(Build(load_image('finishedqfarm.png'), 1, 1, a[1], a[2], all_sprites, type_farm))
        elif a[0] == 'mill':
...
while running:
    if not pause:
        ...
        if warn:
            ...
            if board.grid_pos[1] < 0 and not (
                    (board.grid_pos[0] == 1 or board.grid_pos[0] == 0 or board.grid_pos[0] == 2
            ...
            elif (board.grid_pos[0] == 3 or board.grid_pos[0] == 4 or board.grid_pos[0] == 5) and board.grid_pos[
                1] == -4:
                coords1 = board.iso_poly2[2]
                rx, ry = coords1
                if ((mx > rx + warn_width) or (mx < rx - 100) or (my > ry + 34) or (my < ry - 83)):
        ...
        for event in pygame.event.get():
            ...
            if event.type == type_revolt:
                Revolt(chance_revolt)
                Plauge(victims_of_infection)
