...
class Revolt:
    def __init__(self, chanse):
        ...
        if self.revolt < self.chanse:
            global structures, foodplus, woodplus, stoneplus, ironplus, grids
            structures[-1][0].kill()
...
food, wood, stone, iron, gold, science, population, happiness, days, limit = \
    2200, 400, 200, 0, 0, 0, 1000, 60, 0, 900
...
class BuildQuarry(BUildFarm):
    ...
    def render(self, screen):
        ...
        if self.mouse_in(mousePos):
            pygame.draw.rect(screen, (103, 0, 0), (self.position[0], self.position[1], self.width, self.heigth))
            if pygame.mouse.get_pressed()[0] and self.mouse_in(mousePos):
                global quarry, food, wood, type_quarry, can_build, grids, board
                if food >= 175 and wood >= 125:
...
class BuildMenu:
    def __init__(self, screen, x, y, lx, ly, side, buildx, buildy):
        ...
        self.quarry = ['Каменоломня -', '-150', '-75']
        self.ok = False
        self.lx = lx
        self.ly = ly
        self.side = side
    ...
    def show_list(self):
        ...
        if self.ok:
            if board.grid_pos not in mountains:
                ...
                set_text(self.mill[0], self.x + 10, self.y + 80)
                set_text(self.mill[1], self.x + 130, self.y + 80)
                set_text(self.mill[2], self.x + 130, self.y + 110)
            else:
                if self.side == 1:
                    pygame.draw.polygon(self.screen, (255, 0, 0), [[self.x + 200, self.y],
                                                           [self.x + 200, self.ly + 50], [self.lx, self.ly + 50],
                    ...
                    pygame.draw.polygon(self.screen, (255, 0, 0), [[self.lx, self.y],
                                                           ...
                                                           [self.x, self.y]], 1)
                pygame.draw.rect(self.screen, (0, 0, 0), (self.x, self.y, self.width, self.heigth))
                butmine = BuildIronMine(position=(self.x + 10, self.y + 5), butHeigth=60, butWidth=175, text='')
                ...
                butqu = BuildQuarry(position=(self.x + 10, self.y + 75), butHeigth=60, butWidth=175, text='')
                butqu.render(self.screen)
                self.screen.blit(self.fdres, (self.x + 120, self.y + 80))
                self.screen.blit(self.wdres, (self.x + 120, self.y + 110))
                set_text(self.quarry[0], self.x + 10, self.y + 80)
                set_text(self.quarry[1], self.x + 150, self.y + 80)
                set_text(self.quarry[2], self.x + 150, self.y + 110)
...
def global_changes():
    ...
    if round(food - population) > 1000 and happiness < 101 and limit > population:
        happiness += 1
    elif ((round((food - population) / 100) < 500) or (limit - population < 100)) and happiness > 0:
        happiness -= 1
    if happiness < 50:
        chance_revolt += 1
    elif happiness > 75:
...
pygame.time.set_timer(type_days, 10000)
